...
> 前端为浏览器端 React (JSX) 单页应用，由 Babel 在浏览器中即时编译，无需构建工具；通过 `fetch` 调用后端 RESTful API。以下摘录关键文件。

### 12.1 API 通信封装 (api.js)

统一封装与后端 `http://localhost:8080/api` 的所有请求，包含超时控制、错误处理与响应解析。
...
function buildUrl(path, params) {
  ...
  if (params) {
    Object.entries(params).forEach(([key, value]) => {
      if (!isBlank(value)) url.searchParams.set(key, value);   /* 拼接查询参数 */
    });
  }
  return url.toString();
}
...
async function request(path, options = {}) {
  ...
  const timer = setTimeout(() => controller.abort(), timeoutMs); /* 8s 超时控制 */
  let response;
  try {
  ...
  let data = raw ? JSON.parse(raw) : null;            /* 解析 JSON 响应 */
...
### 12.2 应用主壳与导航 (app.jsx)

定义整个系统的导航菜单结构（账册管理 / 业务操作 / 分析与查询三大组），并作为根组件挂载各功能视图。
...
| 文件 | 职责 |
|------|------|
| `index.html` | 页面入口，加载 React / Babel / GSAP 与各 JSX 模块 |
| `js/api.js` | 后端 API 通信封装（请求、超时、错误处理、字段归一化） |
| `js/app.jsx` | 应用主壳、导航菜单、视图路由 |
| `js/data.jsx` | 前端数据状态管理 |
| `js/ui.jsx` | 通用 UI 组件（按钮、弹窗、表单控件等） |
| `js/views-tables.jsx` | 库存 / 交易 / 进货三大表格视图 |
| `js/views-actions.jsx` | 新建销售、录入进货等业务操作表单 |
| `js/views-query-stats.jsx` | 多条件查询与经营统计可视化 |
| `js/views-welcome.jsx` | 欢迎页 |
| `js/assistant.jsx` | AI 店务助手前端面板 |
| `js/tweaks-panel.jsx` | 界面外观调节面板 |
| `css/styles.css` | 全站样式（中式账册风格） |

---

## 十三、打包说明

本文件夹 `课程设计报告-核心代码打包` 的目录结构：

```
课程设计报告-核心代码打包/
├── 课程设计报告-核心代码汇总.md     ← 本文档
├── backend/        后端 C 语言核心源码（.c / .h）
│   ├── data.c/.h        数据层（加载、保存、ID 生成、查找）
│   ├── product.c/.h     商品管理 CRUD + JSON 解析
│   ├── sale.c/.h        销售管理 + 库存联动 ★核心业务★
│   ├── purchase.c/.h    进货管理 + 库存联动
│   ├── stats.c/.h       统计分析（仪表盘、排行、预警）
│   ├── assistant.c/.h   AI 店务助手
│   ├── server.c         HTTP 服务器与路由分发
│   ├── utils.c/.h       工具函数（JSON 构建、URL 解码等）
│   ├── generate_data.c  示例数据生成程序
│   └── Makefile / build.bat   编译脚本
├── frontend/       前端 React (JSX) 源码
│   ├── index.html
│   ├── js/*.jsx, api.js
│   └── css/styles.css
├── data/           示例数据文件（管道符分隔文本）
│   └── products.txt / sales.txt / purchases.txt
└── launcher.c      一键启动器（编译为 启动系统.exe）
```

**编译运行**：进入 `backend/` 目录执行 `build.bat`（或 `make`）编译出 `server.exe`，运行后浏览器访问前端 `index.html` 即可。

---

*以上为核心代码，涵盖数据结构、数据层、JSON 构建器/解析器、商品管理 CRUD、销售/进货的库存联动逻辑、统计分析算法、HTTP 服务器路由，以及前端 React 单页应用等全部关键技术实现。*
