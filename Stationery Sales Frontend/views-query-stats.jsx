    ...
    <div>
      <div className="page-head">
        <div className="page-head-left">
          ...
          <h1 className="page-title"><span className="cn-num">柒</span>经营统计</h1>
        </div>
        <div className="page-actions">
...
function StatsView({ inventory, transactions, purchases, categories = CATEGORIES, loading = false }) {
  ...
  const [statsLoading, setStatsLoading] = useState(loading);
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const [dashboard, weekly, ranking, inventoryReport, lowStock] = await Promise.all([
        ...
        StationeryApi.getWeeklyStats("橡皮"),
  ...
  const eraserCategory = weekly.category || "橡皮";

  const eraserItems = reportProducts
    .filter(p => p.category === eraserCategory)
    .map(p => ({
      ...p,
      stock: n(p.stock),
      sold_qty: n(p.sold_qty),
      sold_amount: n(p.sold_amount),
      inventory_value: n(p.inventory_value),
    }))
    .sort((a, b) => b.sold_amount - a.sold_amount);

  const rankingRows = (stats.ranking || []).slice(0, 6);
      ...
      {firstLoad ? <StatsSkeleton /> : (
        <div className="stats-strip">
          <div className="stat-cell">
            ...
            <div className="stat-value">{fmtInt(n(dashboard.product_count))}<span className="unit">种</span></div>
            <div className="stat-foot">total products</div>
          </div>
          <div className="stat-cell">
            <div className="stat-label">库存总量</div>
            <div className="stat-value">{fmtInt(n(dashboard.total_stock))}<span className="unit">件</span></div>
            <div className="stat-foot">inventory units</div>
          </div>
          <div className="stat-cell">
            <div className="stat-label">销售总额</div>
            <div className="stat-value vermillion">{fmtMoney(n(dashboard.total_sales))}</div>
            <div className="stat-foot up">gross sales</div>
          </div>
          <div className="stat-cell">
            <div className="stat-label">低库存预警</div>
            <div className="stat-value">{fmtInt(n(stats.lowStock?.total ?? dashboard.low_stock_count))}<span className="unit">项</span></div>
          ...
          <>
            <div className="dash-card dark full">
              <div className="dash-card-head">
                <div>
                  <div className="dash-eyebrow">SPECIAL · 近一周橡皮专题</div>
                  <div className="dash-title">{eraserCategory} / Eraser · Past 7 Days</div>
                </div>
                <div style={{ fontFamily: "var(--mono)", fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted-2)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 32, marginTop: 24 }}>
                <div>
                  <div className="dash-eyebrow" style={{ marginBottom: 12 }}>销售总额 · REVENUE</div>
                  <div className="big-number">{fmtMoney(n(weekly.total_amount))}</div>
                  <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--muted-2)", marginTop: 8, letterSpacing: "0.06em" }}>
                    {fmtInt(n(weekly.total_qty))} 件售出 · 当前库存 {fmtInt(n(weekly.current_stock))} 件
                  </div>
                </div>
                <div>
                  <div className="dash-eyebrow" style={{ marginBottom: 12 }}>本月销售 · MONTH</div>
                  <div className="big-number" style={{ color: "var(--paper)" }}>
                    {fmtMoney(n(dashboard.month_sales))}
                  </div>
                  <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--muted-2)", marginTop: 8, letterSpacing: "0.06em" }}>
                    今日 {fmtMoney(n(dashboard.today_sales))} · 本周 {fmtMoney(n(dashboard.week_sales))}
                  </div>
                </div>
                <div>
                  <div className="dash-eyebrow" style={{ marginBottom: 12 }}>每日销售 · DAILY</div>
            ...
            <div className="dash-card">
              ...
              <div className="top-list">
                {eraserItems.length === 0 && <div className="empty">暂无橡皮统计</div>}
                {eraserItems.map((it, idx) => (
                  <div className="top-row" key={it.id}>
                    <div className="top-rank">{idx + 1}</div>
                    <div>
                      <div className="top-name">{it.name}</div>
                      <div className="top-meta">
                        {it.id} · 当前库存 {fmtInt(it.stock)} 件
                      </div>
                    </div>
                    <div>
                      <div className="top-val">{fmtMoney(it.sold_amount)}</div>
                      <div className="top-meta" style={{ textAlign: "right" }}>{fmtInt(it.sold_qty)} 件</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="dash-card">
              <div className="dash-card-head">
                <div>
                  <div className="dash-eyebrow">RANKING</div>
                  <div className="dash-title">销售排行榜</div>
                </div>
              </div>
              <div className="top-list">
                {rankingRows.length === 0 && <div className="empty">暂无排行数据</div>}
                {rankingRows.map(row => (
                  <div className="top-row" key={row.product_id}>
                    <div className="top-rank">{row.rank}</div>
                    <div>
                      <div className="top-name">{row.name}</div>
                      <div className="top-meta">{row.product_id} · {row.category} · 库存 {fmtInt(n(row.current_stock))}</div>
                    </div>
                    <div>
                      <div className="top-val">{fmtMoney(n(row.total_amount))}</div>
                      <div className="top-meta" style={{ textAlign: "right" }}>{fmtInt(n(row.total_qty))} 件</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="dash-card full">
              <div className="dash-card-head">
                <div>
                  <div className="dash-eyebrow">LEDGER SNAPSHOT · 全店概览</div>
                  <div className="dash-title">各类目库存与累计经营</div>
                </div>
              </div>
              <div className="tbl-wrap" style={{ marginTop: 12 }}>
                <table className="tbl">
                  <thead>
                    <tr>
                      ...
                      <th style={{ textAlign: "right" }}>库存总值</th>
                      <th style={{ textAlign: "right" }}>累计销售</th>
                      <th style={{ textAlign: "right" }}>进货成本</th>
                    </tr>
                  </thead>
                  <tbody>
                    {categoryRows.length === 0 && (
                      <tr><td colSpan="6" className="empty">暂无库存报告</td></tr>
                    )}
                    {categoryRows.map(row => (
                      <tr key={row.name}>
                        <td><CategoryTag value={row.name} /></td>
                        <td className="num-cell">{fmtInt(row.sku)}</td>
                        <td className="num-cell">{fmtInt(row.stock)}</td>
                        <td className="num-cell">{fmtMoney(row.value)}</td>
                        <td className="num-cell">{fmtMoney(row.sold)}</td>
                        <td className="num-cell">{fmtMoney(row.bought)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function HelpView() {
  const sections = [
    ...
    { num: "柒", title: "经营统计", desc: "近一周橡皮销售总额与库存数量专题、橡皮各 SKU 销售明细、分类销售占比、全店账册快照。" },
  ];
  return (
    <div>
      <div className="page-head">
